# Manual de Instação do Conecteup na VPS 
...
```bash
CREATE DATABASE conecteup CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
```
...
25. baixar o repositório do Conecteup

```bash
git clone https://github.com/conecteup/chatup-free chatup-free
```

26. Copiar o env de exemplo para o backend

```bash
cp chatup-freebackend/.env.example chatup-free/backend/.env
```
...
```bash
NODE_ENV=  
BACKEND_URL=https://back.conecteup.com  
FRONTEND_URL=https://ticket.conecteup.com  
PORT=8080  
PROXY_PORT=443  
...
```bash
pm2 start dist/server.js --name chatup-free-backend
```
...
45. 
```bash
REACT_APP_BACKEND_URL=https://back.conecteup.com
REACT_APP_HOURS_CLOSE_TICKETS_AUTO=
REACT_APP_PAGE_TITLE=conecteup
PORT=3333
```
...
```bash
pm2 start server.js --name chatup-free-frontend
```
...
```bash
server {  
  server_name front.conecteup.com;  
  location / {  
...
53. Criar e editar o arquivo chatup-free-frontend com o comando abaixo e prencher com os dados do item 52.

```bash
sudo nano /etc/nginx/sites-available/chatup-free-frontend
```

54. Criar uma cópia do arquivo chatup-free-frontend com o comando abaixo para criar o arquivo para o backend.

```bash
sudo cp /etc/nginx/sites-available/chatup-free-frontend /etc/nginx/sites-available/chatup-free-backend
```
...
```bash
server {  
  server_name back.conecteup.com;  
  location / {  
...
56. editar o arquivo chatup-free-backend com o comando abaixo e prencher com os dados do item 55.

```bash
sudo nano /etc/nginx/sites-available/chatup-free-backend
```
...
60. Criar link simbólico para o arquivo chatup-free-frontend

```bash
sudo ln -s /etc/nginx/sites-available/chatup-free-frontend /etc/nginx/sites-enabled
```

61. Criar link simbólico para o arquivo chatup-free-backend

```bash
sudo ln -s /etc/nginx/sites-available/chatup-free-backend /etc/nginx/sites-enabled
```
